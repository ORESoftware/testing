...
process.once('error', function (e) {
    client_1.log.error('process error:', e && e.message || e);
    process.exit(1);
});
process.once('unhandledRejection', function (e) {
    client_1.log.error('unhandledRejection:', e && e.message || e);
    process.exit(1);
});
process.once('uncaughtException', function (e) {
    client_1.log.error('uncaughtException:', e && e.message || e);
    process.exit(1);
});
new client_1.Client({ port }).ensure().then(function (c) {
    c.lock(key, { ttl: 6000, isViaShell: true }, function (e) {
        if (e) {
        ...
        client_1.log.info(chalk_1.default.green.bold(`${chalk_1.default.italic('Acquired')} lock for key:`), `'${chalk_1.default.blueBright.bold(key)}'`);
        process.exit(0);
    });
});
