...
exports.log = {
    info: console.log.bind(console, chalk_1.default.gray.bold('[live-mutex broker]')),
    error: console.error.bind(console, chalk_1.default.gray.bold('[live-mutex broker]'))
};
...
process.on('warning', function (e) {
    exports.log.error('warning:', e && e.stack || e);
});
...
const removeKeyFromWsLock = function (keys, key) {
...
class Broker {
    constructor(o, cb) {
        ...
        let sigEvent = function (event) {
            return function (err) {
                err && exports.log.error('There was an error:', err);
                if (!callable) {
                    return;
                }
                callable = false;
        ...
        process.on('uncaughtException', function (e) {
            exports.log.error('Uncaught Exception:', e && e.stack || e);
        });
    ...
    ensureNewLockHolder(lck, data) {
        ...
        if (obj = notifyList.shift()) {
            let ws = obj.ws;
            let ttl = we_are_debugging_1.weAreDebugging ? 50000000 : (obj.ttl || this.lockExpiresAfter);
    ...
    lock(data, ws) {
        ...
        const uuid = data.uuid;
        const pid = data.pid;
        const ttl = we_are_debugging_1.weAreDebugging ? 500000000 : (data.ttl || this.lockExpiresAfter);
