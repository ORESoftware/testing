...
impl BaseUrl {
    ...
    pub fn parse(raw: &str) -> Result<Self, DiagnosticError> {
        ...
        if url.query().is_some() || url.fragment().is_some() {
            return Err(DiagnosticError::QueryOrFragment);
        }
        if is_metadata_or_link_local(&url) {
            return Err(DiagnosticError::MetadataEndpoint);
        }
    ...
    /// Returns a bounded, credential-free origin label without path, query, or
    /// fragment data.
    pub fn origin_label(&self) -> String {
        let host = self.0.host_str().unwrap_or("invalid");
        match self.0.port() {
            Some(port) => format!("{}://{}:{port}", self.0.scheme(), host),
            None => format!("{}://{}", self.0.scheme(), host),
        }
    }
...
fn is_metadata_or_link_local(url: &Url) -> bool {
    ...
    if matches!(
        normalized.as_str(),
        "metadata.google.internal" | "metadata.azure.internal" | "169.254.169.254"
    ) {
        return true;
    }
    host.parse::<IpAddr>().is_ok_and(|address| match address {
        IpAddr::V4(address) => address.is_link_local() || address.is_unspecified(),
        IpAddr::V6(address) => address.is_unicast_link_local() || address.is_unspecified(),
    })
}
...
/// Conservative policy for public JSON diagnostics.
...
impl Default for HttpLimits {
    fn default() -> Self {
        Self {
            ...
            response_bytes: ByteLimit::new(4 * 1_024 * 1_024),
        }
    }
}

/// Credential-incapable client bound to one validated diagnostic authority.
#[derive(Clone)]
pub struct PublicDiagnosticClient {
    base: BaseUrl,
    client: DiagnosticClient,
}

impl PublicDiagnosticClient {
    /// Builds a public client with no ambient proxy, redirect, retry, or
    /// credential attachment behavior.
    pub fn new(base: BaseUrl, limits: HttpLimits) -> Result<Self, DiagnosticError> {
        let config = DiagnosticClientConfig::new("ore-mcp-http/0.1")
    ...
    /// slash is accepted for ergonomic endpoint spelling; authority replacement
    /// and traversal remain rejected by [`EndpointPath`].
    pub async fn get_json(&self, path: &str) -> Result<serde_json::Value, DiagnosticError> {
        if path.starts_with("//") {
            return Err(DiagnosticError::InvalidEndpoint);
        }
        let endpoint = EndpointPath::parse(path.strip_prefix('/').unwrap_or(path))?;
        let response = self.client.get_bytes(&self.base, &endpoint).await?;
        serde_json::from_slice(response.body())
            .map_err(|_| DiagnosticError::InvalidJson(response.status()))
    }
...
pub enum DiagnosticError {
    ...
    EmbeddedCredentials,
    /// Query or fragment data was rejected rather than silently discarded.
    QueryOrFragment,
    /// Cloud metadata, link-local, or unspecified endpoints were rejected.
    MetadataEndpoint,
    ...
    InvalidResponse(u16),
    /// A bounded success body was not valid JSON.
    InvalidJson(u16),
...
impl fmt::Display for DiagnosticError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::QueryOrFragment => formatter.write_str("query_or_fragment_rejected"),
            Self::MetadataEndpoint => formatter.write_str("metadata_endpoint_rejected"),
            ...
            Self::InvalidJson(status) => {
                write!(formatter, "invalid_json (status={status})")
            }
...
mod tests {
    ...
    #[test]
    fn accepts_loopback_http_and_rejects_query_fragment() {
        assert_eq!(
            BaseUrl::parse("http://127.0.0.2:8080/base?token=nope#fragment").err(),
            Some(DiagnosticError::QueryOrFragment)
        );
    ...
    #[test]
    fn rejects_metadata_and_authority_replacement() {
        assert_eq!(
            BaseUrl::parse("http://169.254.169.254").err(),
            Some(DiagnosticError::MetadataEndpoint)
        );
