...
pub fn valid_failure_test() {
  result_envelope.Failure(result_envelope.FailureEnvelope(
...
pub fn unicode_code_point_boundary_test() {
  ...
  |> result_envelope.validate
  |> should.equal(
    Error(result_envelope.ContractError("max_length", "$/error/message")),
  )
}
...
pub fn indexed_path_error_test() {
  ...
  |> result_envelope.validate
  |> should.equal(
    Error(result_envelope.ContractError("min_length", "$/error/path/0")),
  )
}

pub fn metadata_bound_error_test() {
  result_envelope.Success(result_envelope.SuccessEnvelope(
    Nil,
    Some(result_envelope.Metadata(None, False, 2_147_483_648)),
  ))
  |> result_envelope.validate
  |> should.equal(
    Error(result_envelope.ContractError("maximum", "$/meta/omitted_bytes")),
  )
}
...
pub fn encodes_failure_test() {
  ...
  |> json.to_string
  |> should.equal(
    "{\"ok\":false,\"error\":{\"code\":\"upstream.timeout\",\"message\":\"timed out\",\"path\":[],\"retryable\":true}}",
  )
}
