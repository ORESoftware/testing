...
pub use ore_mcp_runtime::ServerIdentity as TelemetryIdentity;
...
pub fn init(spec: TelemetrySpec, filter: EnvFilter) -> TelemetryGuard {
    init_parts(spec.name, spec.namespace, spec.version, filter)
}

/// Installs telemetry from the validated runtime identity so service labels do
/// not drift between bootstrap and exporter resources.
pub fn init_identity(identity: &TelemetryIdentity, filter: EnvFilter) -> TelemetryGuard {
    init_parts(
        identity.service_name(),
        identity.service_namespace(),
        identity.version(),
        filter,
    )
}

fn init_parts(name: &str, namespace: &str, version: &str, filter: EnvFilter) -> TelemetryGuard {
    let resource = resource_parts(name, namespace, version);
...
fn resource_parts(name: &str, namespace: &str, version: &str) -> Resource {
    let mut attributes = vec![
        KeyValue::new("service.name", name.to_owned()),
        KeyValue::new("service.namespace", namespace.to_owned()),
...
/// Returns sanitized resource attributes in deterministic key order.
#[must_use]
pub fn sanitize_resource_attributes(raw: &str) -> BTreeMap<String, String> {
    parse_resource_attributes(raw).into_iter().collect()
}
